...
export default function AdminEdit() {
  ...
  const fetchAdmin = async () => {
    try {
      ...
      showToastMessage('Erreur lors du chargement de l\'administrateur', 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await api.put(`/users/admins/${id}/`, formData);
      
      if (response.data) {
        showToastMessage('Administrateur mis à jour avec succès !', 'success');
        setTimeout(() => {
      ...
      if (err.response?.data?.errors) {
        ...
        setError(err.response?.data?.message || 'Erreur lors de la mise à jour de l\'administrateur');
      }
    } finally {
      setSaving(false);
    }
  ...
  const handlePasswordSubmit = async (e) => {
    ...
    try {
      ...
      await api.put(`/users/admins/${id}/password`, {
      ...
      <SuperAdminLayout>
        <div className="flex items-center justify-center py-12">
          <div className="text-center">
            ...
            <p className="mt-4 text-gray-600 dark:text-gray-400">Chargement de l'administrateur...</p>
          </div>
        </div>
      </SuperAdminLayout>
    );
